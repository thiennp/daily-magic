...
Feature: Team dispatch
  ...
  Scenario: Delegate task to own Mac
    Given I am signed in as "test-dispatch-self-1@agentwitch.com"
    And my Mac is paired and online
  ...
  Scenario: Delegate task to colleague with Open policy
    Given company "E2E Open Team" has dispatch policy "Open"
    And "test-dispatch-requester-1@agentwitch.com" is a member
    And "test-dispatch-executor-1@agentwitch.com" is a member with Mac online
    And the executor published a team-visible playbook
    When I sign in as "test-dispatch-requester-1@agentwitch.com"
    And I open the send-task composer
    And I select team dispatch
    And I choose executor "test-dispatch-executor-1@agentwitch.com"
    And I select their Mac and playbook
    And I submit the task
    Then the run should execute on the executor's Mac without approval

  Scenario: Request sensitive work with Approval policy
    Given company "E2E Approval Team" has dispatch policy "Approval"
    And requester "test-dispatch-requester-2@agentwitch.com" is a member
    And executor "test-dispatch-executor-2@agentwitch.com" is a member with Mac online
    When I sign in as "test-dispatch-requester-2@agentwitch.com"
  ...
  Scenario: Manager approves pending dispatch
    Given a pending approval exists for requester "test-dispatch-requester-2@agentwitch.com"
    When I sign in as company admin "test-dispatch-admin-2@agentwitch.com"
