...
| **3 Efficient memory** | Mac run memory (recent prompt/output pairs), playbooks, RAG in harness; cloud `capability_improvements`; **git worktree verdict** appended to Mac run report `details` after each dispatch | Selective memory (success signals, dedupe, no unsafe replay); team-scoped memory policy |
...
**Known gaps (developers):** raw run memory without outcome filtering; some improvement paths not fully wired Mac ↔ cloud — see [project-composition.md](../architecture/project-composition.md) (memory section) and feature `KNOWN_ISSUES.md`.
