...
7. **Split process** — **done (phase 1)** — `process-host` slice: `AgentWitchHostMode` (`monolith` \| `bridge-external` \| `live-external`) from `AGENT_WITCH_EXTERNAL_BRIDGE` / `AGENT_WITCH_EXTERNAL_LIVE`; `startAgentWitchClient` skips in-process AWB wake server and/or AWL local app when set (default unchanged). **Phase 2 (done):** install-time orchestration — when the installer shell has truthy `AGENT_WITCH_EXTERNAL_BRIDGE` / `AGENT_WITCH_EXTERNAL_LIVE`, the generated main LaunchAgent plist copies those env keys to the hub client; optional auxiliary LaunchAgents register **`${prefix}-wake`** (`command/wake.sh` → bundled `bridge` CLI / AWB) and **`${prefix}-live`** (`command/live.sh` → bundled `local-app` CLI / AWL). Default monolith install still retires auxiliary agents. Bundle subcommands: `bridge`, `local-app`. `macLifecycle` adapter re-exports via bridge `process-control` public-api + `@agent-witch/install-macos-launch` (scripts remain thin shims). **Deferred:** moving revive/watchdog helpers fully off `scripts/`; standalone AWL without hub-owned connection health.
