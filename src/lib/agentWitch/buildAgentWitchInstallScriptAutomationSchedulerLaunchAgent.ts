...
cat > "\${APP_DIR}/${AGENT_WITCH_COMMAND_DIR_NAME}/automation-scheduler.sh" <<'AUTOMATION_SCHEDULER_EOF'
#!/usr/bin/env bash
set -euo pipefail
# Scheduled automations run in-process inside the main Agent Witch client.
exit 0
AUTOMATION_SCHEDULER_EOF
chmod +x "\${APP_DIR}/${AGENT_WITCH_COMMAND_DIR_NAME}/automation-scheduler.sh"
