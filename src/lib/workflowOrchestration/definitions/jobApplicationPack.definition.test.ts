...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/jobApplicationPack.definition";

describe("job-application-pack official workflow definition", () => {
  ...
  it("references workflow form fields in agent prompts", () => {
    const agentPrompts = OFFICIAL_WORKFLOW_DEFINITION.nodes
      .filter((node) => node.kind === "agent")
      .map((node) => node.promptSection)
      .join("\n");

    expect(agentPrompts).toContain("targetRole");
    expect(agentPrompts).toContain("companyName");
    expect(agentPrompts).toContain("resumeFolderPath");
    expect(agentPrompts).toContain("jobDescription");
    expect(agentPrompts).toContain("applicationHistoryPath");
  });
  ...
  it("alternates human and agent nodes", () => {
    const kinds = OFFICIAL_WORKFLOW_DEFINITION.nodes.map((node) => node.kind);
    expect(kinds).toEqual(["human", "agent", "human", "agent", "human"]);
  });
});
