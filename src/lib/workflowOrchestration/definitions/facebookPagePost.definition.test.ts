...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/facebookPagePost.definition";
...
describe("facebook-page-post official workflow definition", () => {
  it("aligns human nodes with harness operator steps", () => {
    const template = findCapabilityTemplateById("facebook-page-post");
  ...
  it("references workflow form fields in agent prompt sections", () => {
    ...
    WORKFLOW_FIELD_KEYS.forEach((fieldKey) => {
      expect(agentPrompts).toContain(fieldKey);
    });
  });
