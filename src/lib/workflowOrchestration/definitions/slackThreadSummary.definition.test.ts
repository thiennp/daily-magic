...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/slackThreadSummary.definition";

describe("slack-thread-summary official workflow definition", () => {
  ...
  it("references workflow form fields in agent steps", () => {
    ...
    expect(agentPrompts).toMatch(/\bthread\b/);
    expect(agentPrompts).toMatch(/\baudience\b/);
    expect(agentPrompts).toMatch(/\bgoal\b/);
  });
