...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/meetingNotesActions.definition";

describe("meeting-notes-actions official workflow definition", () => {
  ...
  it("references workflow form fields in agent prompt sections", () => {
    ...
    expect(agentSections).toContain("meetingTitle");
    expect(agentSections).toContain("notes");
    expect(agentSections).toContain("attendees");
  });
