...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/tenantSupportReply.definition";

describe("tenant-support-reply official workflow definition", () => {
  ...
  it("references workflow form fields in agent prompt sections", () => {
    ...
    expect(agentPrompts).toContain("tenantMessage");
    expect(agentPrompts).toContain("leaseNotesPath");
    expect(agentPrompts).toContain("issueType");
    expect(agentPrompts).toContain("tenantLogPath");
  });
