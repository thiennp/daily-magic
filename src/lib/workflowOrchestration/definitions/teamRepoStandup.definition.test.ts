...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/teamRepoStandup.definition";

describe("team-repo-standup official workflow definition", () => {
  ...
  it("references workflow form fields in agent steps", () => {
    ...
    expect(agentText).toContain("repoPath");
    expect(agentText).toContain("branch");
    expect(agentText).toContain("since");
  });
