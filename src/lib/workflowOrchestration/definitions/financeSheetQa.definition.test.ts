...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/financeSheetQa.definition";

describe("finance-sheet-qa official workflow definition", () => {
  ...
  it("references workflow form fields in agent prompt sections", () => {
    ...
    expect(agentPrompts).toContain("sheetUrl");
    expect(agentPrompts).toContain("sheetSummary");
    expect(agentPrompts).toContain("userQuestion");
    expect(agentPrompts).toContain("tabOrRange");
    expect(agentPrompts).toContain("notesPath");
  });
