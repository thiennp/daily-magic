...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/compareOptions.definition";

describe("compare-options official workflow definition", () => {
  ...
  it("references workflow form fields in agent prompt sections", () => {
    ...
    expect(agentPrompts).toMatch(/optionA/);
    expect(agentPrompts).toMatch(/optionB/);
    expect(agentPrompts).toMatch(/criteria/);
  });
