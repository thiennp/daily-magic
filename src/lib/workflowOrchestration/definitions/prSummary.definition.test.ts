...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/prSummary.definition";

describe("pr-summary official workflow definition", () => {
  ...
  it("references workflow form fields in agent prompt sections", () => {
    ...
    expect(agentPrompts).toMatch(/context/);
    expect(agentPrompts).toMatch(/change/);
    expect(agentPrompts).toMatch(/audience/);
    expect(agentPrompts).toMatch(/testNotes/);
  });
