...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/sprintRecap.definition";

describe("sprint-recap official workflow definition", () => {
  ...
  it("references workflow form fields in agent prompt sections", () => {
    ...
    expect(agentPrompts).toContain("sprintName");
    expect(agentPrompts).toContain("shipped");
    expect(agentPrompts).toContain("missed");
    expect(agentPrompts).toContain("nextFocus");
  });
