...
import { OFFICIAL_WORKFLOW_DEFINITION } from "@/lib/workflowOrchestration/definitions/oneOnOnePrep.definition";

describe("one-on-one-prep official workflow definition", () => {
  ...
  it("references workflow form fields in agent prompt sections", () => {
    ...
    expect(agentPrompts).toContain("person");
    expect(agentPrompts).toContain("sinceLast");
    expect(agentPrompts).toContain("topics");
    expect(agentPrompts).toContain("feedback");
  });
