...
export const formatAgentAccessLlmsText = (origin: string): string => {
  const urls = buildAgentAccessUrls(origin);
  const guide = buildAgentAccessGuideline(origin);
  ...
  return [
    ...
    `> Grokbot and other agents register and use ${AGENT_WITCH_PRODUCT_NAME} with no human account. Open the guideline, then call the HTTP API. Do not ask the human for an email or password.`,
