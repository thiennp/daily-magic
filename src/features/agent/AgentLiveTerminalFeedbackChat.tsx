...
export default function AgentLiveTerminalFeedbackChat({
  ...
  const deferredSubmit = useAgentLiveTerminalFeedbackDeferredSubmit({ onSubmit });
  ...
  const content = (
    <>
      {isAnswerMode ? (
        <>
          <p className="text-sm font-medium text-amber-800 dark:text-amber-200">Your Mac agent needs input</p>
          <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">{pendingQuestion}</p>
        </>
      ) : isSteppedComposer ? null : (
        <p className="text-sm font-medium text-gray-800 dark:text-white/90">Follow up in the live terminal</p>
      )}
      ...
      {queuedCount > 0 ? (
        <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
          {queuedCount} follow-up{queuedCount === 1 ? "" : "s"} queued while your Mac agent is working.
        </p>
      ) : null}
      {queueNotice !== null ? (
        <p className="mt-2 text-xs text-brand-600 dark:text-brand-400">{queueNotice}</p>
      ) : null}
  ...
  return isSteppedComposer ? (
    ...
    <div className="mt-4 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900/40">{content}</div>
  );
}
