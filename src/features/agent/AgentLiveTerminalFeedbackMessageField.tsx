...
export default function AgentLiveTerminalFeedbackMessageField({
  ...
  const field = (
    <textarea
      value={message}
      autoFocus={autoFocus}
      aria-label={isAnswerMode ? undefined : "Follow-up message for the live terminal"}
      onChange={(event) => onMessageChange(event.target.value)}
      rows={3}
      placeholder={isAnswerMode ? "Reply to your Mac agent…" : "Ask a follow-up or add context for the next step…"}
      aria-invalid={hasMessageError}
      className={isAnswerMode ? `mt-2 ${textareaClassName}` : textareaClassName}
    />
  );
  ...
  return (
    <>
      {isAnswerMode ? (
        <label className="mt-3 block text-sm text-gray-700 dark:text-gray-300">Your answer{field}</label>
      ) : (
        <div className={isSteppedComposer ? "" : "mt-3"}>{field}</div>
      )}
      {hasMessageError ? (
        <span className="mt-1 block text-sm text-rose-600 dark:text-rose-400">{messageError}</span>
      ) : null}
    </>
  );
}
