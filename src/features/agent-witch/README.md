...
Bundle version: bump `AGENT_WITCH_INSTALL_BUNDLE_VERSION` when install scripts change (currently **53**). Heartbeat `system.ack` advertises that version so connected Macs auto-update when local `install-version.json` differs.
