...
describe("marketingSurfaceClasses.constant", () => {
  ...
  it("MARKETING-004 uses darker eyebrows for WCAG AA on marketing home (UX-P1d)", () => {
    expect(MARKETING_EYEBROW_TEXT_CLASSES).toContain("text-zinc-700");
    expect(MARKETING_EYEBROW_TEXT_CLASSES).not.toContain("text-zinc-500");
    expect(MARKETING_EYEBROW_TEXT_CLASSES).not.toContain("text-zinc-600");
  });

  it("MARKETING-005 uses AA-muted and placeholder tokens on light marketing surfaces", () => {
    expect(MARKETING_TEXT_MUTED_CLASSES).toContain("text-zinc-600");
    expect(MARKETING_TEXT_MUTED_CLASSES).not.toContain("text-zinc-500");
    expect(MARKETING_INPUT_BASE_CLASSES).toContain("placeholder:text-zinc-600");
